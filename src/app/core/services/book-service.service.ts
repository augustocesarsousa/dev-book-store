...
export class BookService {
  ...
  filterBook(): void {
    ...
    if (this.filter.categories.length > 0) {
      this.bookListFiltered = this.filterBookListByCategory(
        this.bookListFiltered
      );
    }
    if (this.filter.price > 0) {
      if (this.filter.price <= 200)
        this.bookListFiltered = this.filterBookListByPriceLess(
          this.bookListFiltered
        );
      if (this.filter.price > 200)
        this.bookListFiltered = this.filterBookListByPriceMore(
          this.bookListFiltered
        );
    }
  ...
  private filterBookListByName(bookList: IBook[]): IBook[] {
    return bookList.filter(
      (book: IBook) =>
        book.name.toLowerCase().indexOf(this.filter.name.toLowerCase()) > -1
    );
  }
